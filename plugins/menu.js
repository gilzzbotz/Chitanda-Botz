 ...
 *Powered By @${'nekopoi.care'.split('@')[0]}*`
 ...
     }) 
     if (teks == '404') {
  let cute = await conn.resize(await (await fetch(`https://telegra.ph/file/04da65ab862cb6f8cfa58.jpg`)).buffer(), 300, 200) 
  let ori = `┏━━━ꕥ〔 *${botname}* 〕ꕥ━⬣
...
/*await conn.send2ButtonDoc(m.chat, '──────────[ *DASHBOARD* ]──────────', text, 'OWNER', '.owner', 'Rules', '.rules', m, { 
 ...
     //conn.sendFile(m.chat, pp, 'menu.jpg', text.trim(), m).catch(_ => conn.reply(m.chat, text.trim(), m))
     let res = await fetch(`https://github.com/saipulanuar/Api-Github/raw/main/sound/sound40.mp3`)
    bzz = await res.buffer()
  conn.sendFile(m.chat, bzz, 'haori.mp3', null, m, true, {
type: 'audioMessage', 
ptt: true, contextInfo:{ externalAdReply: { title: `💌 Hitung Mundur : ${ohari} Hari ${ojam} Jam ${onet} Menit ${detek} Detik`, body: `${pickRandom(['Follow Tiktok My Bestie'])}`, sourceUrl: 'https://www.tiktok.com/@raraharsita2', thumbnail: await (await fetch('https://i.ibb.co/jfZVKmC/babi2.jpg')).buffer(),}} 
     })
} catch (e) { 
